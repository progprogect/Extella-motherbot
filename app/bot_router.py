...
async def _process(utg, bot, msg: dict, session):
    ...
    if raw_text in ('/start', '/help'):
        if exps:
            ...
            if local_n and not bot.user_target_id:
                conn_note = f'\n\n\u26a0\ufe0f {local_n} \u044d\u043a\u0441\u043f. \u043d\u0443\u0436\u043d\u0430 \U0001f4bb. /connect \u0434\u043b\u044f \u043f\u043e\u0434\u043a\u043b\u044e\u0447\u0435\u043d\u0438\u044f.'
            await utg.send_message(cid,
                f'\U0001f44b \u0411\u043e\u0442 Extella AI | \u0444\u0443\u043d\u043a\u0446\u0438\u0439: {len(exps)}\n{lines}'
    ...
    logger.info(f'bot={bot.id} expert={best.expert_name} type={mt} lang={lang}')
    ...
    if furl:
        ...
        if not is_filler and pp != uk: params[pp] = text
    else:
        params[pp] = text
    ...
    if etype == 'local':
        if not bot.user_target_id or not bot.user_extella_token_enc:
            await utg.send_message(cid,
                f'\u26a0\ufe0f \u042d\u043a\u0441\u043f\u0435\u0440\u0442 <b>{best.expert_name}</b> \u0442\u0440\u0435\u0431\u0443\u0435\u0442 \U0001f4bb.\n'
                '\u041f\u043e\u0434\u043a\u043b\u044e\u0447\u0438 \u043c\u0430\u0448\u0438\u043d\u0443 \u0447\u0435\u0440\u0435\u0437 /connect')
            return
        user_tok = decrypt_token(bot.user_extella_token_enc, settings.secret_key)
        client = ExtellaClient(user_tok)
        result = await client.run_expert(best.expert_name, params,
                                          timeout=90, target=bot.user_target_id)
    else:
        result = await extella.run_expert(best.expert_name, params, timeout=90)
...
async def _respond(utg, cid, result, multi, name):
    label = f'\U0001f9e0 <i>{name}</i>\n\n' if multi else ''
    if result.get('status') == 'error':
        await utg.send_message(cid, f'\u26a0\ufe0f {result.get("message","?")}')
        return
    inner = result.get('result', result)
    if not inner:
        await utg.send_message(cid, label + '\u041d\u0435 \u043f\u043e\u043b\u0443\u0447\u0438\u043b \u043e\u0442\u0432\u0435\u0442.')
        return
    if isinstance(inner, dict) and inner.get('status') == 'error':
        await utg.send_message(cid, f'\u26a0\ufe0f {inner.get("message","?")}')
        return
    if isinstance(inner, dict):
        iu = (inner.get('result_url') or inner.get('image_url')
               or inner.get('output_url') or inner.get('output_image_url'))
        if iu:
            r = await utg.send_photo(cid, iu, caption=label + inner.get('message', '\u2705'))
            if not r.get('ok'):
